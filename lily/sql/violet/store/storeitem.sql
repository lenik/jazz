...
--\import violet.store.storeodr
    ...
    create sequence storeitem_seq start with 1000;
    create table storeitem(
        id          bigint primary key default nextval('storeitem_seq'),
    ...
    -- trigger support
        ...
        begin
            for v in select count(*) "rows", sum(qty) "qty_sum", sum(total) "total_sum"
                from storeitem where odr=new.odr
            loop
                c := v."rows";
                cqty := v.qty_sum;
                ctotal := v.total_sum;
        ...
        create trigger storeitem_insert
            after insert or delete on storeitem
            for each row execute procedure update_storeodr_stat();

        create trigger storeitem_update
            after update of qty, price on storeitem
            for each row execute procedure update_storeodr_stat();
