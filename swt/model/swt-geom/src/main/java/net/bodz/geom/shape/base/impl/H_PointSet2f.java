...
public class H_PointSet2f {
    ...
    public static class PointRefIterator
            extends _PointIterator
            implements Iterator<Point2f> {
    ...
    /*
     * public static final Point2f[] convertToArray(Object data) { if (data == null) return null; if
     * (data instanceof Point2f) return new Point2f[] { (Point2f) data, }; if (data instanceof
     * Point2f[]) return (Point2f[]) data;
     * 
     * Class clazz = data.getClass(); ArrayList<Point2f> list = new ArrayList<Point2f>(); if
     * (clazz.isArray()) { int length = Array.getLength(data); for (int i = 0; i < length; i++) {
     * Object v = Array.get(data, i); if (v instanceof Point2f) list.add((Point2f) v); else if (v
     * instanceof Number && i + 1 < length) { Object v2 = Array.get(data, i + 1); assert v2
     * instanceof Number; list.add(new Point2f.XY(((Number) v).floatValue(), ((Number)
     * v2).floatValue())); } else throw new IllegalArgumentException(
     * "Invalid point in point-array-def"); } } else { throw new IllegalArgumentException(
