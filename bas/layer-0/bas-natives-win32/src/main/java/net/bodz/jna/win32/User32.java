...
public interface User32
     ...
     * specified window. The dimensions are given in screen coordinates that are relative to the
     * upper-left corner of the screen.
     */
    boolean GetWindowRect(HWND hWnd, RECT rect);

    /**
     * The GetWindowTextLength function retrieves the length, in characters, of the specified
     * window's title bar text (if the window has a title bar). If the specified window is a
     * control, the function retrieves the length of the text within the control. However,
     * GetWindowTextLength cannot retrieve the length of the text of an edit control in another
     * application.
     */
    int GetWindowTextLength(HWND hWnd);

    /**
     * The GetWindowText function copies the text of the specified window's title bar (if it has
     * one) into a buffer. If the specified window is a control, the text of the control is copied.
     * However, GetWindowText cannot retrieve the text of a control in another application.
     * 
     * @param lpString
     *            Pointer to the buffer that will receive the text. If the string is as long or
     *            longer than the buffer, the string is truncated and terminated with a NULL
     *            character.
     * @param nMaxCount
     ...
     * specified point. The search is restricted to immediate child windows. Grandchildren, and
     * deeper descendant windows are not searched.
     * <p>
     * To skip certain child windows, use the ChildWindowFromPointEx function.
     * 
     * @return The return value is a handle to the child window that contains the point, even if the
     *         child window is hidden or disabled. If the point lies outside the parent window, the
     *         return value is NULL. If the point is within the parent window but not within any
     ...
     * child windows. The search is restricted to immediate child windows. Grandchildren and deeper
     * descendants are not searched.
     */
    HWND ChildWindowFromPointEx(HWND hwndParent, POINTByValue pt, int uFlags);

    /**
     * RealChildWindowFromPoint treats HTTRANSPARENT areas of a standard control differently from
     * other areas of the control; it returns the child window behind a transparent part of a
     * control. In contrast, ChildWindowFromPoint treats HTTRANSPARENT areas of a control the same
     * as other areas. For example, if the point is in a transparent area of a groupbox,
     * RealChildWindowFromPoint returns the child window behind a groupbox, whereas
     * ChildWindowFromPoint returns the groupbox. However, both APIs return a static field, even
     * though it, too, returns HTTRANSPARENT.
     */
    HWND RealChildWindowFromPoint(HWND hwndParent, POINTByValue ptParentClientCoords);

    /**
     * The return value is a handle to the window that contains the point. If no window exists at
     * the given point, the return value is NULL. If the point is over a static text control, the
     * return value is a handle to the window under the static text control.
     */
