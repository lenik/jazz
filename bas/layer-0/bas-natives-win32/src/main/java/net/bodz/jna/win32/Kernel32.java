...
public interface Kernel32
    ...
    /**
     * The file system preserves and enforces access control lists (ACL). For example, the NTFS file
    ...
    /**
     * @param rootPathName
     *            [in, optional] A pointer to a string that contains the root directory of the
     *            volume to be described.
     * 
     *            If this parameter is NULL, the root of the current directory is used. A trailing
     ...
     *            [out] A pointer to a buffer that receives the name of a specified volume. The
     *            maximum buffer size is MAX_PATH+1.
     * @param volumeNameSize
     *            [in] The length of a volume name buffer, in TCHARs. The maximum buffer size is
     *            MAX_PATH+1.
     * 
     *            This parameter is ignored if the volume name buffer is not supplied.
     * @param volumeSerialNumber
     *            [out, optional] A pointer to a variable that receives the volume serial number.
     * 
     ...
     *            when a hard disk is formatted. To programmatically obtain the hard disk's serial
     *            number that the manufacturer assigns, use the Windows Management Instrumentation
     *            (WMI) Win32_PhysicalMedia property SerialNumber.
     * @param maximumComponentLength
     *            [out, optional] A pointer to a variable that receives the maximum length, in
     *            TCHARs, of a file name component that a specified file system supports.
     * 
     *            A file name component is the portion of a file name between backslashes.
     * 
     *            The value that is stored in the variable that *lpMaximumComponentLength points to
     *            is used to indicate that a specified file system supports long names. For example,
     *            for a FAT file system that supports long names, the function stores the value 255,
     *            rather than the previous 8.3 indicator. Long names can also be supported on
     *            systems that use the NTFS file system.
     * @param fileSystemFlags
     *            [out, optional] A pointer to a variable that receives flags associated with the
     ...
     *            [out] A pointer to a buffer that receives the name of the file system, for
     *            example, the FAT file system or the NTFS file system. The maximum buffer size is
     *            MAX_PATH+1.
     * @param fileSystemNameSize
     *            [in] The length of the file system name buffer, in TCHARs. The maximum buffer size
     *            is MAX_PATH+1. This parameter is ignored if the file system name buffer is not
     *            supplied.
     */
    int GetVolumeInformationA(String rootPathName, Buffer volumeNameBuffer, int volumeNameSize,
            IntByReference volumeSerialNumber, IntByReference maximumComponentLength, IntByReference fileSystemFlags,
