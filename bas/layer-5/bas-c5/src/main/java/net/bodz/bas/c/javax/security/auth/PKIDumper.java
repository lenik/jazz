...
public class PKIDumper {
    ...
    public PKIDumper(IPrintOut out, int detail) {
        if (out == null)
                    ...
                    if (store != null) {
                        try {
                            ...
                            for (String alias : Iterables.otp(store.aliases())) {
                                dumpStoreEntry(prefix + "  ", store, alias, null, detail - 1);
                            }
    ...
    public void dumpKeyStore(String prefix, KeyStore keyStore, int detail)
            throws KeyStoreException {
        for (String alias : Iterables.otp(keyStore.aliases()))
        ...
        if (key instanceof X509Key) {
            ...
            try {
                ...
                if (encodedInternal != null) {
                    ...
                    if (Arrays.equals(encoded, encodedInternal))
                        out.println("(same)");
                    else
    ...
    void dumpBytes(String prefix, byte[] bytes, int detail) {
        ...
        if (detail < 1) {
            ...
            String hex = HexCodec.getInstance().encode(bytes, 0, part);
