...
public interface IKeyPatternLikeMakeRule4<Tp extends IKeyPatternLike<Param, K>, Param, K, //
    ...
    default MakeAction<T> compile(@NotNull T target, @NotNull IMakeSession session)
        ...
        SimpleMakeRule4<T, K, TT, U, UK, UT, V, VK, VT, W, WK, WT, X, XK, XT> rule = SimpleMakeRule4.<T, K, TT, U, UK, UT, V, VK, VT, W, WK, WT, X, XK, XT>builder()//
                .priority(this.getPriority())//
                .fn(fn).build();

        MakeAction<T> instance = new MakeAction<>(rule, target, input1, input2, input3, input4);
        return instance;
    }
