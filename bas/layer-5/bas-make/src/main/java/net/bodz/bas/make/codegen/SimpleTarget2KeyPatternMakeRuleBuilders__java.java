...
public class SimpleTarget2KeyPatternMakeRuleBuilders__java
        extends Class__java {

    @Override
    public QualifiedName getQName() {
        return QualifiedName.parse("net.bodz.bas.make.pattern.key.SimpleTarget2KeyPatternMakeRuleBuilders");
    }
    ...
    public void build(JavaCodeWriter out)
        ...
        out.println();
        out.printf("public abstract class SimpleTarget2KeyPatternMakeRuleBuilders<S, Tp extends ITarget2KeyPattern<Param, T, TK, TT>, Param, TK, T extends IKeyData<TK, TT>, TT> {\n");

        out.println();
        out.enter();
        {
            out.printf("public abstract BiConsumer<S, ITarget2KeyPatternMakeRule<Tp, Param, TK, T, TT>> getApply();\n");
            ...
            for (int inputCount = 0; inputCount <= maxCount; inputCount++) {
                ...
                out.printf("SimpleTarget2KeyPatternMakeRule%d.Builder<%s> input(%s) {\n", //
                        inputCount, //
                        builderTypeVars, //
                        Naming.inputParams(inputCount, "s", "", "s"));
                out.enter();
                {
                    out.printf("return SimpleTarget2KeyPatternMakeRule%d.<%s>builder()//\n", inputCount, builderTypeVars);
                    out.enter();
                    {
                        out.enter();
                        {
