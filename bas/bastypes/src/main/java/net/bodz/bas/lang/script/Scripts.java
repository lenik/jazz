...
public class Scripts {
    ...
    public static <T> ScriptClass<T> getScriptClass(Class<?> origClass, Object dynamicImpl) throws ScriptException {
        ...
        if (sclass == null) {
            ...
            if (scType == null)
                throw new ScriptException(LangNLS.getString("Scripts.nosclassFor") //$NON-NLS-1$
                        + origClass);
    ...
    public static <T> ScriptClass<T> getScriptClass(Class<?> origClass) throws ScriptException {
        return getScriptClass(origClass, null);
    }

    public static <T> ScriptClass<T> getScriptClass(Object dynamicImpl) throws ScriptException {
    ...
    static ScriptClass<?> newScriptClass(Class<? extends ScriptClass<?>> scType, Class<?> origClass, Object dynamicImpl) {
        ...
        try {
            if (dynamicImpl == null) {
                ...
                staticCtor = (Constructor<ScriptClass<?>>) scType.getConstructor(STATIC_CTOR);
                sclass = staticCtor.newInstance(origClass);
            } else {
                Constructor<ScriptClass<?>> dynamicCtor;
                dynamicCtor = (Constructor<ScriptClass<?>>) scType.getConstructor(DYNAMIC_CTOR);
            ...
            throw new RuntimeException(LangNLS.getString("Scripts.errNewInst"), e); //$NON-NLS-1$
        }
        return sclass;
    }
     ...
     *             if <code>clazz</code> is null
     */
    public static <T> ScriptClass<T> convertClass(Class<?> clazz) throws ScriptException {
        assert clazz != null;
    ...
    public static <T> ScriptField<T> convertField(Field field) throws ScriptException {
    ...
    public static <T> ScriptMethod<T> convertMethod(Method method) throws ScriptException {
