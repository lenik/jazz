...
public class ManagedProcessTest {
    ...
    public ManagedProcessTest() {
        ...
        if (lapiota != null) {
            File f = new File(lapiota, "bin/mtpulse.exe"); //$NON-NLS-1$
            if (f.exists())
                mtpulse = f;
        }
    }
    ...
    static class Cap extends _IOCallback {
        ...
        public void recvErr(byte[] b, int off, int len) throws IOException {
            String s = "!" + new String(b, off, len, encoding) + " "; //$NON-NLS-1$ //$NON-NLS-2$
            buf.print(s);
    ...
    void test(String cmd, String expected, int expectedExit, String... inputs) throws Exception {
        ...
        String cap_u = cap.toString().replace("\r", ""); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void testOutFirst() throws Exception {
        test("C a a a a a a a a a a a a a a a Xx", //$NON-NLS-1$
                "-a -a -a -a -a -a -a -a -a -a -a -a -a -a -a !x ", 0); //$NON-NLS-1$
    }

    @Test
    public void testErrFirst() throws Exception {
        test("C Xx Xx Xx Xx Xx Xx Xx Xx Xx Xx Xx Xx Xx Xx Xx a", //$NON-NLS-1$
                "!x !x !x !x !x !x !x !x !x !x !x !x !x !x !x -a ", 0); //$NON-NLS-1$
    }

    @Test
    public void testCont() throws Exception {
        test("C a Xb Oc ?10", "-a !b -c ", 10); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    public void testIn() throws Exception {
        test("C a E b E c", "-a -x\n -b -y\n -c ", 0, "x", "y", "z"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
    }

    public static void main(String[] args) throws Throwable {
        Process catproc = Processes.shellExec("cat"); //$NON-NLS-1$
