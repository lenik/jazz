...
public abstract class AbstractPolygon2f
    ...
    public float index(float x, float y) {
        ...
        ILineRef2f edge = edge(i);
        return i + edge.index(x, y);
    }

    public float index(IPointRef2d point) {
    ...
    public ILineRef2f edgeRef(int index) {
        int n = pointCount();
        IPointRef2d start = pointRef(index++);
        IPointRef2d end = pointRef(index % n);
        return start.lineTo(end);
    }

    public ILineRef2f edgeRef(int index1, int index2) {
        int n = pointCount();
        if (index2 == (index1 + 1) % n)
            return edge(index1);
        IPointRef2d p1 = pointRef(index1);
        IPointRef2d p2 = pointRef(index2);
        return p1.lineTo(p2);
    }

    public ILineRef2f.Static edge(int index) {
        int n = pointCount();
        // x()
        IPointRef2d start = point(index++);
        IPointRef2d end = point(index % n);
        return start.lineTo(end);
    }

    public ILineRef2f.Static edge(int index1, int index2) {
        int n = pointCount();
        if (index2 == (index1 + 1) % n)
            return edge(index1);
        IPointRef2d p1 = point(index1);
        IPointRef2d p2 = point(index2);
        return p1.lineTo(p2);
    }
    ...
    public void addIndex(float index) {
        ...
        if (start == end)
            return;
        IPointRef2d point = pointRef(start).lineTo(pointRef(end)).point(index - start);
        addPoint(end, point);
    }

    public void addCross(ILineRef2f line) {
        int n = pointCount();
        if (n < 2)
            return;
        int offset = isClosed() ? n - 1 : n - 2;
        IPointRef2d ep = pointRef((offset + 1) % n);
        for (int i = offset; i >= 0; i--) {
            IPointRef2d p = pointRef(i);
            ILineRef2f edge = p.lineTo(ep);
            IPointRef2d x = edge.intersectsAt(line);
            if (x != null)
                addPoint(i + 1, x);
            ep = p;
        }
    ...
    public List<StaticPoint2f> intersectsAt(ILineRef2f line, int max) {
    ...
    public List<Integer> intersectsAtIndex(ILineRef2f line, int max) {
    ...
    public List<StaticPoint2f> intersectsAt(ILineRef2f line) {
        return intersectsAt(line, Integer.MAX_VALUE);
    }

    public List<Integer> intersectsAtIndex(ILineRef2f line) {
        return intersectsAtIndex(line, Integer.MAX_VALUE);
    }

    public int intersectsCount(ILineRef2f line, int maxTest) {
        assert maxTest > 0;
        int n = pointCount();
        int c = 0;
        for (int i = 0; i < n; i++)
    ...
    public final int intersectsCount(ILineRef2f line) {
        return intersectsCount(line, Integer.MAX_VALUE);
    }

    public boolean intersects(ILineRef2f line) {
    ...
    protected void swapPoint(int indexA, int indexB) {
        IPointRef2d a = pointRef(indexA);
        IPointRef2d b = pointRef(indexB);
        setPoint(indexA, b);
        setPoint(indexB, a);
    }
        ...
        entire: while (true) {
            edge: for (int i = 2; i < n; i++) {
                findcut: for (int j = 0; j < i - 1; j++) {
                    ILineRef2f I = edge(i);
                    ILineRef2f J = edge(j);
                    IPointRef2d x = I.intersectsAt(J);
                    if (x == null)
                        continue;
                    assert j < i;
                    // j-+-j'-----------i-+-i'
    ...
    static final class TdTempPolygon
        ...
        public TdTempPolygon(List<IPointRef2d> points) {
            super(points);
            selfCuts = selfCuts();
        }
    ...
    public final void addPoint(IPointRef2d point) {
        int n = pointCount();
        addPoint(n, point);
    }
    ...
    public IShape2f crop(IPointRef2d baseHalfPlane, Vector2f normal) {
        ArrayList<IPointRef2d> points = new ArrayList<IPointRef2d>();

        int n = pointCount();

        int off = isOpened() ? 0 : n - 1;
        IPointRef2d pre = pointRef(off);
        boolean lastOut = pre.crop(baseHalfPlane, normal) == null;

        ILineRef2f cut = baseHalfPlane.halfPlane(normal);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (lastOut == out) {
                ...
                IPointRef2d x = pre.lineTo(p)._intersectsAt(cut);
                assert x != null;
                if (out) { // in -> out
                    points.add(x);
                } else { // out -> in
