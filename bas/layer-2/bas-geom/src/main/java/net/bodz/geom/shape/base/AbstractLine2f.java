...
public abstract class AbstractLine2f
        ...
        implements ILineRef2f, Serializable {
    ...
    public void x0(float x0) {
        throw new ReadOnlyAttributeException(this, "x0");
    }

    public void y0(float y0) {
        throw new ReadOnlyAttributeException(this, "y0");
    }

    public void x1(float x1) {
        throw new ReadOnlyAttributeException(this, "x1");
    }
    ...
    public StaticPoint2f _intersectsAt(ILineRef2f line) {
        float k1 = gradient();
        float b1 = y();
    ...
    public boolean _intersects(ILineRef2f line) {
        return _intersectsAt(line) != null;
    }

    public StaticPoint2f intersectsAt(ILineRef2f line) {
    ...
    public float distanceExtended(IPointRef2d point) {
    ...
    public float distanceExtended(float x, float y) {
        ...
        /*
         * dist < 0 : left to the line dist = 0 : on the line dist > 0 : right to the line
         */
    ...
    public final boolean containsHP(IPointRef2d point) {
        Vector2f N = toVector();
        N.normalize();
    ...
    @Override
    public ILineRef2f crop(IPointRef2d base, Vector2f normal) {
        assert base != null;
        assert normal != null;
        ...
        IPointRef2d cut = P.addCopy(PQ.toScale(t));
        if (den > 0) {
            // PQ enters into
            if (t <= 0)
                return this;
                ...
                return new ILineRef2f.Static(cut, Q);
        } else {
            // PQ leaves out
            if (t < 0)
                return null;
                ...
                return new ILineRef2f.Static(P, cut);
        }
    }

    @Override
    public ILineRef2f crop(IRectangle2f rect) {
        return (ILineRef2f) cropConvexPolygon(rect);
    }

    @Override
    public ILineRef2f crop(float x0, float y0, float x1, float y1) {
        return crop(new IRectangle2f.StaticLeft(x0, y0, x1, y1));
    }

    public ILineRef2f convertToLine2f() {
        return null;
    }
    ...
    @Override
    public ILineRef2f.Static snapshot() {
        return new ILineRef2f.Static(p0(), p1());
    }
