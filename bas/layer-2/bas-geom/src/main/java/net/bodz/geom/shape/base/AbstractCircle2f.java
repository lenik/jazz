...
public abstract class AbstractCircle2f
     ...
     * (shape.getClass().isAssignableFrom(Point2f.class)) return addWithScale((Point2f)shape); if
     * (contains(shape)) return false; return false; }
     * 
     * public boolean addWithScale(float x, float y) { if (radius() <= 0) { setCenterX(x);
     * setCenterY(y); setRadius(Float.MIN_VALUE); return true; }
     * 
     * float cx = centerX(); float cy = centerY(); float radius = radius();
     * 
     * float dx = x - cx; float dy = y - cy; float D2 = dx * dx + dy * dy;
     * 
     * // already contains the point(x, y) if (D2 <= radius() * radius()) return false;
     * 
     * float D = (float)Math.sqrt(D2); float R = (D + radius) / 2;
     * 
     * setCenterX(((R - radius) * cx + R * x) / D); setCenterY(((R - radius) * cy + R * y) / D);
     * setRadius(R); return true; }
     * 
