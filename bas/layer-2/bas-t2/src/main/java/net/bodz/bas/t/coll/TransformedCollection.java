...
public class TransformedCollection<S, T>
    ...
    public TransformedCollection(Collection<S> source, Function<S, T> transformer, Class<T> targetType,
            Function<T, S> untransformer) {
        this.source = source;
    ...
    public static <S, T> TransformedCollection<S, T> transform(Collection<S> source, Function<S, T> transformer,
            Class<T> targetType) {
        return new TransformedCollection<S, T>(source, transformer, targetType);
    }

    public static <S, T> TransformedCollection<S, T> transform(Collection<S> source, Function<S, T> transformer,
            Class<T> targetType, Function<T, S> untransformer) {
        return new TransformedCollection<S, T>(source, transformer, targetType, untransformer);
    }
