    ...
    public V get(Object key) {
        ...
        if (qVal == null)
            if (!qMap.containsKey(key))
                if (pDels == null || !pDels.contains(key))
                    return pMap.get(key);
        return qVal;
    }
    ...
    class PIterator extends PrefetchedIterator<Entry<K, V>> {
        ...
        protected Entry<K, V> fetch() {
            while (pItr.hasNext()) {
                ...
                if (pDels != null && pDels.contains(pNxtKey))
                    continue;
                return pNxt;
            }
            return end();
