...
import net.bodz.bas.api.exceptions.IllegalArgumentTypeException;
import net.bodz.bas.api.functor.Pred1;
...
public class Arrays2 {
    ...
    @SuppressWarnings("unchecked")
    public static <A> A map(A array, MapFunction<?> mapFunction) {
        MapFunction<Object> _function = (MapFunction<Object>) mapFunction;
        return (A) map((Object[]) array, _function);
    }

    public static <T> T[] map(T[] array, MapFunction<T> mapFunction) {
        if (array == null)
            return null;
