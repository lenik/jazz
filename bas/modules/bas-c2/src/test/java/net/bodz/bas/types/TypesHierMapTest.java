...
public class TypesHierMapTest {
    ...
    {
        orig.put(r(Cat.class, Dog.class), "Cat, Dog"); //$NON-NLS-1$
        orig.put(r(C.class, Dog.class), "C, Dog"); //$NON-NLS-1$
        orig.put(r(Cat.class, D.class), "Cat, D"); //$NON-NLS-1$
        orig.put(r(C.class, D.class), "C, D"); //$NON-NLS-1$
        orig.put(r(CI.class, Dx.class), "CI, Dx"); //$NON-NLS-1$
        orig.put(r(CI.class, DxKI.class), "CI, DxKI"); //$NON-NLS-1$
        orig.put(r(CJz.class, DxKI.class), "CJz, DxKI"); //$NON-NLS-1$
        orig.put(r(CJz.class, DIJy.class), "CJz, DIJy"); //$NON-NLS-1$
        orig.put(r(CJzKI.class, DIJ.class), "CJzKI, DIJ"); //$NON-NLS-1$
    }

    void dumpOrig() {
        System.out.println("Orig entry order: "); //$NON-NLS-1$
        for (Entry<Class<?>[], Object> e : orig.entrySet()) {
            Class<?>[] key = e.getKey();
            System.out.println(Types.joinNames(", ", true, key)); //$NON-NLS-1$
        }
        System.out.println();
    }
    ...
    public void test1() {
        ...
        assertEquals("CI, DxKI", map.floor(r(CI.class, DxKIx.class))); //$NON-NLS-1$

        children = Collections2.toList(map.ceilings(r(CI.class, D.class)));
        assertEquals(2, children.size());
        assertEquals("CI, Dx", children.get(0)); //$NON-NLS-1$
        assertEquals("CI, DxKI", children.get(1)); //$NON-NLS-1$

        children = Collections2.toList(map.ceilings(r(CJ.class, I.class)));
        assertEquals(3, children.size());
        // DIJy extends DIJ, but CJz order take precedence.
        assertEquals("CJz, DIJy", children.get(0)); // DIJy //$NON-NLS-1$
        assertEquals("CJz, DxKI", children.get(1)); //$NON-NLS-1$
        assertEquals("CJzKI, DIJ", children.get(2)); // DIJ //$NON-NLS-1$
