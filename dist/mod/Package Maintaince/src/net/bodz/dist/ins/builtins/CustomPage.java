...
public class CustomPage extends ConfigPage {
    ...
    public CustomPage(ISession _session, Composite parent, int style) {
                    ...
                    browseButton.addSelectionListener(new SelectionAdapter() {
                        ...
                        public void widgetSelected(SelectionEvent e) {
                            DirectoryDialog dialog = new DirectoryDialog(
                                    getShell(), SWT.NONE);
                            dialog.setText("Select the directory for "
        ...
        basedirsComp.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
                false, 2, 1));
        basedirsComp.setText("Install Locations:");
        ...
        installSizeLabel.setText("Install Size: ");
    ...
    public void validate() throws ValidateException {
        ...
        for (int i = 0; i < baseDirs.length; i++) {
            ...
            if (dirFile.isFile())
                throw new ValidateException(dirText, "file exists");
            else if (!dirFile.exists()) {
                Interaction iact = session.getInteraction();
                boolean confirmed = iact.confirm("Create the directory?",
                        "The directory " + dirFile
                                + " isn't existed, shall I create it?");
                if (!confirmed) {
                    throw new SlientValidationException(dirText);
                } else if (!dirFile.mkdirs())
                    throw new ValidateException(dirText, "Can't create "
                            + dirFile);
            }
